...
const margin4 = { top: 20, right: 20, bottom: 100, left: 100 };
...
const legendSvg = d3.select("#my_legend")
  .append("svg")
...
d3.csv("/data/scatter_data2.csv").then((data) => {
  ...
  const groupedRegionalIndicator = Object.keys(groupedData).map((key) => [key, groupedData[key]]);
  console.log(groupedRegionalIndicator);

  const totalRegionalIndicator = groupedRegionalIndicator.map((d, i) => ({ region: d[1][0].region, value: d[0] }));
  console.log(totalRegionalIndicator);

  const getMinMaxValue = function (colName) { return d3.extent(data, (d) => +d[colName]); };
  ...
  const colorPalette = ["#a30b71", "#8FBFCC", "#26265c", "#ffd20a", "#008080", "#FF1D33", "#226D7B", "#002E6B"];
  ...
  const legend = legendSvg.selectAll(".legend")
    ...
    .enter()
    .append("g")
    .attr("transform", (d, i) => "translate(" + i % n * itemWidth + "," + Math.floor(i / n) * itemHeight + ")")
    .attr("class", "legend");
