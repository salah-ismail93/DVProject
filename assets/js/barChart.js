class BarChart {
  ...
  initVis() {
    ...
    vis.yAxisCall = d3.axisLeft().ticks(4).tickFormat(customTickFormat);
  ...
  updateVis() {
    ...
    vis.x.domain(
      vis.vaccineData.map(function (d) {
        return d.iso_code;
      })
    );
    ...
    vis.g
      ...
      .on("mouseover", function (d) {
        vis.tooltip.transition().duration(100).style("opacity", 0.9);
        vis.tooltip.html(
          "<h3> Country - " +
            iso_map[d.iso_code] +
            "</h3>" +
            "<p> Dose : " +
            customTickFormat(d.dose) +
            "</p>"
        );
        vis.tooltip
          ...
          .on("mouseout", function (d) {
            vis.tooltip.transition().duration(100).style("opacity", 0);
          });
      })
      .on("mouseout", function (d) {
        vis.tooltip.transition().duration(100).style("opacity", 0);
