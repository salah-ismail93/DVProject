class StackedBarChart {
  ...
  initVis() {
    ...
    vis.columns = {
      per_million: [
        "first_dose_per_million",
        "last_dose_per_million",
        "booster_dose_per_million",
      ],
  ...
  wrangleData() {
    ...
    vis.vaccineData = d3
      ...
      .rollup(function (d) {
        var new_row = {};
        new_row["first_dose"] = d3.sum(
          d,
          (data) => data[vis.columns[vis.variable][0]]
        );
        new_row["second_dose"] = d3.sum(
          d,
          (data) => data[vis.columns[vis.variable][1]]
        );
        new_row["booster_dose"] = d3.sum(
          d,
          (data) => data[vis.columns[vis.variable][2]]
        );
        return new_row;
      })
      .entries(barData)
      .map(function (data) {
        ...
        d.total_dose =
          data.value.first_dose +
          data.value.second_dose +
          data.value.booster_dose;
        return d;
      });
    if (vis.sorting == "maxmin") {
      vis.vaccineData.sort(function (a, b) {
        return b.total_dose - a.total_dose;
      });
    } else {
      vis.vaccineData.sort(function (a, b) {
        return a.total_dose - b.total_dose;
      });
    }
  ...
  updateVis() {
    ...
    vis.y.domain(
      this.vaccineData.map(function (d) {
        return d.location;
      })
    );
    vis.x
      .domain([
        0,
        d3.max(vis.vaccineData, function (d) {
          return d.total_dose;
        }),
      ])
      .nice();
    ...
    vis.text = vis.g
      .selectAll("g.vaccine")
      .selectAll("text.data_text")
      .data(vis.vaccineData);
    vis.text
      .exit()
      .remove()
      .attr("y", function (d) {
        return vis.y(d.location) + 15;
      })
      .attr("x", function (d) {
        return vis.x(d.total_dose) + 5;
      })
    ...
    vis.g
      ...
      .on("mouseover", function (d) {
        ...
        vis.tooltip.html(
          "<h3> Country - " +
            ...
            "</h3>" +
            "<p> First Dose : " +
            customTickFormat(d.data.a) +
            "</p>" +
            "<p> Second Dose : " +
            customTickFormat(d.data.b) +
            "</p>" +
            "<p> Booster Dose : " +
            customTickFormat(d.data.c) +
            "</p>"
        );
        vis.tooltip
      ...
      .on("mouseout", function (d) {
        vis.tooltip.transition(vis.t).duration(200).style("opacity", 0);
      });
  }
  addLegend() {
    const vis = this;
