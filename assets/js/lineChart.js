class LineChart {
  ...
  initVis() {
    ...
    //For Legend
    vis.legend = vis.svg.append("g").attr("id", "line_legend");
    ...
    vis.line = d3
      .line()
      .x(function (d) {
        return vis.x(xParseTime(d.date));
      })
      .y(function (d) {
        return vis.y(d[vis.yColumn]);
      });
    //For tooltip
          ...
          .map((day) =>
            day.values.reduce(
              (maxValue, current) => {
                ...
                maxValue["total_cases"] = d3.max([
                  maxValue["total_cases"],
                  current["total_cases"],
                ]);
                maxValue["total_cases_million"] = d3.max([
                  maxValue["total_cases_million"],
                  current["total_cases_million"],
                ]);
                maxValue["total_deaths"] = d3.max([
                  maxValue["total_deaths"],
                  current["total_deaths"],
                ]);
                maxValue["total_death_million"] = d3.max([
                  maxValue["total_death_million"],
                  current["total_death_million"],
                ]);
                return maxValue;
              },
              {
    ...
    if (vis.operation == "sortMax") {
      vis.covidData = vis.covidData.sort(function (a, b) {
        return d3.descending(
          d3.max(a.value, (d) => d[vis.yColumn]),
          d3.max(b.value, (d) => d[vis.yColumn])
        );
      });
    } else if (vis.operation == "sortMin") {
      vis.covidData = vis.covidData.sort(function (a, b) {
        return d3.ascending(
          d3.min(a.value, (d) => d[vis.yColumn]),
          d3.min(b.value, (d) => d[vis.yColumn])
        );
      });
    }
  ...
  updateVis() {
    ...
    // update scales
    vis.x.domain(
      d3.extent(vis.all_date, function (d) {
        return xParseTime(d);
      })
    );
    ...
    vis.country = vis.g.selectAll(".countries").data(vis.covidData);
        ...
        vis.tooltip
          ...
          .html(function (data) {
            ...
            if (data.value.find((d) => d.date == strDate) != undefined) {
              axis_value = data.value.find((d) => d.date == strDate)[
                vis.yColumn
              ];
            }
