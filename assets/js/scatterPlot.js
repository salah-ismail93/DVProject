class ScatterPlot {
  ...
  initVis() {
    ...
    //For Scales
    vis.x = d3.scaleLog().range([0, vis.WIDTH]).domain([0.1, 40000]);
  ...
  wrangleData() {
    ...
    vis.covidData = d3
      ...
      .map(function (d) {
        return {
          iso_code: d.value.iso_code,
          location: d.key,
          continent: d.value.continent,
          case_per_million: d.value.case_per_million,
          tests_per_thousand: d.value.tests_per_thousand,
          death_per_million: d.value.death_per_million,
        };
      });
  ...
  updateVis() {
    ...
    vis.y
      .domain([
        0,
        d3.max(vis.covidData, function (d) {
          return d[vis.yColumn];
        }),
      ])
      .nice();
      ...
      .on("mouseover", function (d) {
        ...
        vis.tooltip.html(
          "<h3>" +
            ...
            "</h3>" +
            "<p>" +
            vis.headings[vis.xColumn] +
            " : " +
            customTickFormat(d[vis.xColumn]) +
            "</p>" +
            "<p>" +
            vis.headings[vis.yColumn] +
            " : " +
            customTickFormat(d[vis.yColumn]) +
            "</p>" +
            "<p>" +
            vis.headings[vis.sizeColumn] +
            " : " +
            customTickFormat(d[vis.sizeColumn]) +
            "</p>"
        );
        vis.tooltip
